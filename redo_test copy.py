...
X = df.drop(columns=['call_price', 'delta', 'theta', 'vega', 'rho']).values
Y = df.drop(columns=['strike', 'underlying', 'maturity',
            'volatility', 'interestrate', 'delta', 'theta', 'vega', 'rho'])
GREEKS = df.drop(columns=['strike', 'underlying', 'maturity',
                 'volatility', 'interestrate', 'call_price'])
...
for m in range(num_epoches):
    ...
    if m == 10000 or m == 100000 or m == 500000 or m == 1000000:
        torch.save(model.state_dict(),
                   'moneyness_model/auto_diff_greeks+' + str(m) + '.ckpt')
    ...
    if m % 100 == 0:
        ...
        for i in range(100):
            ...
            # "strike,underlying,maturity,volatility,interestrate,call_price,delta,theta,vega,rho\n")
            test_greeks.append(
                np.array([S.grad.item(), T.grad.item(), sigma.grad.item(), r.grad.item()]))

        pred_prices = []
        pred_greeks = []
        # run model on validation set
        for i in range(len(test_x_vals)):
            moneyness = test_x_vals[i][1] / test_x_vals[i][0]
            # data_row = test_x_vals[i][2:]
            # data_row.insert(0, moneyness)
            data_row = torch.tensor([moneyness, test_x_vals[i][2].item(), test_x_vals[i][3].item(), test_x_vals[i][4].item()])
            data_row.requires_grad = True
            z = model(data_row)
            pred_prices.append(z.item())
            z.sum().backward()
            pred_greeks.append(data_row.grad)
        ...
        # new dimensions so this is all wrong, how to convert moneyness derivative back to underlying for delta
...
# plt.title('delta residual MSE: ' + str(delta_error))
# plt.savefig('moneyness_model/delta_residual_plot' + '.png', bbox_inches="tight")
# plt.clf()

# plt.scatter(theta, res_theta, label='theta test error')
# plt.title('theta residual MSE: ' + str(theta_error))
# plt.savefig('moneyness_model/theta_residual_plot' + '.png', bbox_inches="tight")
# plt.clf()

# plt.scatter(vega, res_vega, label='vega test error')
# plt.title('vega residual MSE: ' + str(theta_error))
# plt.savefig('moneyness_model/vega_residual_plot' + '.png', bbox_inches="tight")
# plt.clf()

# plt.scatter(rho, res_rho, label='rho test error')
# plt.title('rho residual MSE: ' + str(theta_error))
# plt.savefig('moneyness_model/rho_residual_plot' + '.png', bbox_inches="tight")
# plt.clf()
