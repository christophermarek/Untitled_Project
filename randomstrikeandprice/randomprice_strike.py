...
def gen_dataset(size):
    ...
    for i in range(size):
        ...
        # generated underylying / strike based off this
        ...
        # Strike price
        K = torch.tensor(random.uniform(75, 100), requires_grad=True)
        
        
        # Underlying price Delta: {S.grad}
        S = torch.tensor(random.uniform(50, 125), requires_grad=True)
...
for m in range(num_epoches):
    ...
    if m == 10000 or m == 100000 or m == 500000 or m == 1000000 or m== 2000000 or m==3000000:
        torch.save(model.state_dict(), 'autodiffwunderlying' + str(m) + '.ckpt')
...
torch.save(model.state_dict(), 'trainingdone+' + str(num_epoches) + '.ckpt')
...
# plt.savefig('final_test_price_residual' + '.png', bbox_inches="tight")
# plt.clf()
