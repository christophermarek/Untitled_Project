...
Y = df.drop(columns=['strike', 'underlying', 'maturity', 'volatility', 'interestrate', 'call_price','theta','delta','rho'])
...
for m in range(num_epoches):
    ...
    if m % 100 == 0:
        ...
        for i in range(100):
            ...
            test_prices.append(sigma.grad.item())
...
for i in range(int(num_epoches* 0.2)):
    ...
    [K.item(), S.item(), T.item(), sigma.item(), r.item()], dtype=torch.float32))
    val_prices.append(sigma.grad.item())
...
plt.xlabel('pred vega')
plt.ylabel('actual vega')
plt.savefig('sigma_output/final_test_price' + '.png', bbox_inches="tight")
plt.clf()
