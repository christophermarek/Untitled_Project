...
Y = df.drop(columns=['strike', 'underlying', 'maturity', 'volatility', 'interestrate', 'dividends', 'call_price','delta','theta','rho', 'epsilon'])
...
for m in range(num_epoches):
    ...
    if m == 10000 or m == 100000 or m == 500000 or m == 1000000 or m == 1500000:
        torch.save(model.state_dict(), 'vega' + str(m) + '.ckpt')

torch.save(model.state_dict(), 'vega.ckpt')
